...
class Articles extends Admin_Controller {
	...
	public function __construct()
	{
		...
		/* Title Page :: Common */
		$this->page_title->push(lang('menu_articles'));
		$this->data['pagetitle'] = $this->page_title->show();

		/* Breadcrumbs :: Common */
		$this->breadcrumbs->unshift(1, lang('menu_articles'), 'admin/articles');
	}
	...
	public function index()
	{
		...
		else
		{
			...
			/* Load Template */
			$this->template->admin_render('admin/articles/index', $this->data);
		}
	}
	...
	{
		/* Breadcrumbs */
		$this->breadcrumbs->unshift(2, lang('menu_articles_create'), 'admin/articles/create');
		$this->data['breadcrumb'] = $this->breadcrumbs->show();
		...
		/* Validate form input */
		$this->form_validation->set_rules('first_name', 'lang:articles_firstname', 'required');
		$this->form_validation->set_rules('last_name', 'lang:articles_lastname', 'required');
		//$this->form_validation->set_rules('email', 'lang:articles_email', 'required|valid_email|is_unique['.$tables['articles'].'.email]');
		$this->form_validation->set_rules('phone', 'lang:articles_phone', 'required');
		$this->form_validation->set_rules('company', 'lang:articles_company', 'required');
		$this->form_validation->set_rules('password', 'lang:articles_password', 'required|min_length[' . $this->config->item('min_password_length', 'ion_auth') . ']|max_length[' . $this->config->item('max_password_length', 'ion_auth') . ']|matches[password_confirm]');
		$this->form_validation->set_rules('password_confirm', 'lang:articles_password_confirm', 'required');
		...
		else
		{
			...
			/* Load Template */
			$this->template->admin_render('admin/articles/create', $this->data);
		}
	}
